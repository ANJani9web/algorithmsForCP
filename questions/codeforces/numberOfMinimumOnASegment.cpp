...
struct SegTree {
    ...
    // Node1 query1(int start, int end, int index, int left, int right) { // Never change this
	// 	if (start > right || end < left)
	// 		return Node();
	// 	if (start >= left && end <= right)
	// 		return tree[index];
	// 	int mid = (start + end) / 2;
	// 	Node1 l, r, ans;
	// 	l = query1(start, mid, 2 * index, left, right);
	// 	r = query1(mid + 1, end, 2 * index + 1, left, right);
	// 	ans.merge(l, r);
	// 	return ans;
	// }
    ...
    // Node1 make_query1(int left,int right){
    //     return query1(0,n-1,1,left,right);
    // }
};
...
struct Node{
	...
	void merge(Node &l, Node &r) { // Merge two child nodes
		...
		if(l.val==r.val) count = r.count+l.count;
		else if(l.val<r.val) count = l.count;
		else count = r.count;
	}
};
...
struct Node1{
   ...
   void merge(Node1& l,Node1& r){
    count = count + l.val==mini;
    count = count + r.val==mini;
   }
};
...
int main(){
  ...
  for(int i=0;i<q;i++){
    ...
    if(x==1){
        ...
        //l--;
        r--;
        //mini = s.make_query(l,r).val;
		Node ans1 = s.make_query(l,r);
		cout<<ans1.val<<" "<<ans1.count<<endl;
        // cout<<mini<<" "<<s.make_query1(l,r).count<<endl;
